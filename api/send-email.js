/**
 * DAITRA Serverless Email Dispatcher
 * Securely handles email notifications from a server context to protect the Brevo API key.
 * Supports sending notifications to both the boutique owner (new order captured) and the customer (order confirmation).
 */
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (body.order) {
      ...
      // Fallback/backward compatibility if order is sent directly in request body
      order = body;
      recipientType = 'owner';
      recipientEmail = null;
    }
    ...
    const OWNER_EMAIL = 'yakshbarot597@gmail.com'; // Local test recipient for owner alerts
    ...
    // Dynamic HTML content building for the email

/**
 * DAITRA Transactional Email Service
 * Integrates Brevo SMTP Transactional API to send order emails.
 * Supports:
 * 1. sendOrderAlertEmail(order) - alerts the shop owner of a new capture.
 * 2. sendCustomerConfirmationEmail(order) - sends a confirmation receipt to the shopper.
 */

// Load API key from Vite environment variables for local fallback mode
const BREVO_API_KEY = import.meta.env.VITE_BREVO_API_KEY || '';
const OWNER_EMAIL = 'yakshbarot597@gmail.com';
const SENDER_EMAIL = 'yakshbarot597@gmail.com'; // Verified sender in Brevo
const SENDER_NAME  = 'Yaksh Barot';

/**
 * Sends a notification email alert to the store owner
 */
export async function sendOrderAlertEmail(order) {
  // First, attempt to send through the secure serverless backend endpoint
  try {
    console.log('Attempting secure backend email dispatch for Owner alert...');
    ...
    if (backendResponse.status !== 404) {
      ...
      console.log('Backend email endpoint response (Owner Alert):', data);
      if (backendResponse.ok) {
        return { success: true, messageId: data.messageId, source: 'backend' };
      } else {
        return { success: false, error: data.error || 'Serverless email failed' };
      }
    }
    console.log('Backend endpoint returned 404. Falling back to direct client-side SMTP dispatch...');
  } catch (backendError) {
    console.warn('Backend endpoint unreachable, falling back to client-side SMTP dispatch:', backendError);
  }

  // Fallback: Direct client-side Brevo API request
  return directBrevoSend(order, 'owner');
}

/**
 * Sends a confirmation receipt email to the customer
 */
export async function sendCustomerConfirmationEmail(order) {
  if (!order.customerInfo.email) {
    console.warn('Skipping customer confirmation email: Customer email is missing.');
    return { success: false, error: 'Customer email missing.' };
  }

  // First, attempt to send through the secure serverless backend endpoint
  try {
    console.log(`Attempting secure backend email dispatch for Customer confirmation to ${order.customerInfo.email}...`);
    const backendResponse = await fetch('/api/send-email', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
    ...
    if (backendResponse.status !== 404) {
      const data = await backendResponse.json();
      console.log('Backend email endpoint response (Customer Conf):', data);
      if (backendResponse.ok) {
        return { success: true, messageId: data.messageId, source: 'backend' };
      } else {
        return { success: false, error: data.error || 'Serverless email failed' };
  ...
  // Fallback: Direct client-side Brevo API request
  return directBrevoSend(order, 'customer');
}

/**
 * Direct client-side SMTP dispatch helper (Vite fallback)
 */
async function directBrevoSend(order, recipientType) {
  if (!BREVO_API_KEY) {
    return {
      success: false,
      error: 'Brevo API configuration error: Key missing in client-side environment (VITE_BREVO_API_KEY).'
    };
  }

  const isOnline = order.paymentType !== 'Cash on Delivery (COD)';
  const targetEmail = recipientType === 'customer' ? order.customerInfo.email : OWNER_EMAIL;
  const targetName = recipientType === 'customer' ? order.customerInfo.name : 'DAITRA Owner';
  ...
  // Helper to resolve absolute URLs for images in email client
  ...
  try {
    console.log(`📧 Sending ${recipientType} email → ${targetEmail}`);
    ...
    if (response.ok) {
      console.log(`✅ Email sent to ${targetEmail} — Message ID: ${data.messageId}`);
      return { success: true, messageId: data.messageId, source: 'brevo' };
    } else {
      console.error(`❌ Brevo rejected email (${recipientType}):`, JSON.stringify(data));
      return { success: false, error: data.message || 'Brevo API error', details: data };
    }
  } catch (error) {
    console.error(`❌ Network error sending email (${recipientType}):`, error.message);
